...
void kmpSoma(char T[], char P[], int tamT, int tamP, int &soma)
{
	...
	while(i < tamT)
	{
		if(T[i] == P[j]) // match
		{
			j++;
			i++;
			if(j == tamP) // encontrei o padrao no texto
			{
				soma++;
				j = lps[j - 1];
			}
...
int main()
{
	...
	int tamT, tamP = 2;
	int n;
	int soma; //somar a quantidade de vezes encontrado
	int maior = -1;//vetor aux para salvar soma
	char res[60];
	...
	for (int i =0; i<n; i++){
		soma =0;
		cin.getline(titulo, 60);
		tamT= strlen(titulo);
		kmpSoma(titulo,padrao,tamT,tamP,soma);
		if (soma > maior ){
			maior = soma;
			strcpy(res, titulo);
		}
