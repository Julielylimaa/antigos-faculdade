...
--
INSERT INTO trabalha_em VALUES (1,1 , 12);
...
WHERE SSN = SSN_Empregado;

#4 
SELECT AVG(Salario) FROM Empregado WHERE Endereco LIKE "%São Paulo%" AND DNumero_Departamento = 1;

#5
SELECT sup.PNome AS Supervisor, emp.PNome AS Empregado, (sup.Salario - emp.Salario) AS Diferenca_Salarial
FROM Empregado AS sup, Empregado AS emp
WHERE emp.SSN_Supervisor = sup.SSN;

#6
SELECT emp.PNome, pro.PNome, tr.Horas 
FROM Empregado AS emp, Projeto AS pro, Trabalha_em AS tr 
WHERE tr.SSN_Empregado = emp.SSN AND tr.PNumero_Projeto = pro.PNumero;