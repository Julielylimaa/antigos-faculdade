...
float area_poligono(ponto v[],int n)
{
	...
	for(int i = 0;i < n-2;i++)
	{
		cout << "Area do setor " << i << " = " << fixed << setprecision(3)<<area_triangulo(v[0],v[i+1],v[i+2]) << endl;
	}
	return 0;
}
...
int main (){
	int n; //numeros de pontos 
	list <ponto> pilha;
	
	cin >> n;
	ponto coordenadas[n];//coordenadas
	for(int i=0; i<n; i++){
